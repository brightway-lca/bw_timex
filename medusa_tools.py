...
def create_demand_timing_dict(timeline: pd.DataFrame, demand: dict) -> dict:
    """
    Generates a dictionary, which maps producer (key) to reference timing (currently YYYYMM) (value) for specific demands. 
        ...
        if not previous_producer_node['database'] in database_date_dict.values():
            ...
            for database, share in row.interpolation_weights.items():
                ...
                datapackage.add_persistent_vector(
                        matrix="technosphere_matrix",
                        name=uuid.uuid4().hex,
                        data_array=np.array([share], dtype=float), #TODO: check for non-unitary processes, maybe multiply with production amount?
                        indices_array=np.array(
