...
In addition to the `representative_date`, `premise` databases also carry metadata on what IAM scenario they reflect, e.g., "REMIND-EU NDC", similar to the `scenario` metadata shown above. `bw_timex` can use this data as well, e.g., when comparing different scenarios; see [Step 2](build_process_timeline.md).
