...
class DynamicCharacterization:
    ...
    def characterize_dynamic_inventory(
        ...
        self.available_characterization_functions = (
            {  # Strings must match the "name" of the flow in the database
        ...
        for _, row in self.dynamic_lci_df.iterrows():
            if (
                flow_mapping[row.flow]
                not in self.available_characterization_functions.keys()
            ):
                warnings.warn(
                    f"There is no dynamic characterization function for {row.flow}, so it will be ingored."
                )
                continue
            ...
            if (
                ...
                if (
                    ...
                    radiative_forcing_ghg = self.available_characterization_functions[
                        flow_mapping[row.flow]
                    ](row, period=TH)
    ...
    def characterize_co2(
        ...
        decay_multipliers: np.ndarray = np.array(
            [radiative_efficiency_kg * self.IRF_co2(year) for year in range(period)]
        )
