...
class DynamicCharacterization():
    ...
    def characterize_dynamic_inventory(self, 
        ...
        self.available_characterization_functions = { # Strings must match the "name" of the flow in the database
            "carbon dioxide": self.characterize_co2, 
            "methane": self.characterize_ch4, 
            "nitrous oxide": self.characterize_n2o,
            "carbon monoxide": self.characterize_co_levasseur, 
            }
        
        flow_mapping = {}
        for flow in self.available_characterization_functions.keys():
            try:
        ...
        for _, row in self.dynamic_lci_df.iterrows():
            if flow_mapping[row.flow] not in self.available_characterization_functions.keys():
                warnings.warn(f"There is no dynamic characterization function for {row.flow}, so it will be ingored.")
                continue
            ...
            if type_of_method == "radiative_forcing": #radiative forcing in W/m2
                ...
                if not fixed_TH: #fixed_TH = False: conventional approach, emission is calculated from t emission for the length of TH
                    self.characterized_inventory = pd.concat([ self.characterized_inventory, self.available_characterization_functions[flow_mapping[row.flow]](row, period = TH)])
                    ...
                    if len(end_TH_FU_list) > 1:
                        warnings.warn(f"There are multiple functional units with different timings. The first one ({str(end_TH_FU_list[0])}) will be used as a basis for the fixed time horizon in dynamic characterization.")
                        
                    end_TH_FU = datetime.strptime(str(end_TH_FU_list[0]), time_res_dict[self.temporal_grouping])
                    ...
                    self.characterized_inventory = pd.concat([self.characterized_inventory, self.available_characterization_functions[flow_mapping[row.flow]](row, period = new_TH)])

            if type_of_method == "GWP": #scale radiative forcing to GWP [kg CO2 equivalent]
                if not fixed_TH: #fixed_TH = False: conventional approach, emission is calculated from t emission for the length of TH
                    radiative_forcing_ghg = self.available_characterization_functions[flow_mapping[row.flow]](row, period = TH)
                    ...
                    self.characterized_inventory = pd.concat([self.characterized_inventory, row_df], ignore_index=True)
                    ...
                    radiative_forcing_ghg = self.available_characterization_functions[flow_mapping[row.flow]](row, period = new_TH) #indidvidual emissions are calculated for t_emission until t_FU + TH
            ...
            if "date" in self.characterized_inventory:
                self.characterized_inventory.sort_values(by="date", ascending=True, inplace=True)
                self.characterized_inventory.reset_index(drop=True, inplace=True)

            if cumsum and "amount" in self.characterized_inventory:
                self.characterized_inventory["amount_sum"] = self.characterized_inventory["amount"].cumsum() #TODO: there is also an option for cumulative results in the characterization functions themselves. Rethink where this is handled best and to avoid double cumsum
    ...
    def characterize_n2o(self, series, period: int = 100, cumulative=False) -> pd.DataFrame:
        ...
        radiative_efficiency_kg = radiative_efficiency_ppb * M_air / M_n2o * 1e9 / m_atmosphere # W/m2/kg-CH4; 
    ...
    def characterize_co_levasseur(self, series, period: int = 100, cumulative=False) -> pd.DataFrame:
        """
    ...
    def characterize_n2o_levasseur(self, series, period: int = 100, cumulative=False) -> pd.DataFrame:
        """
