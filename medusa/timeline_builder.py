...
def create_grouped_edge_dataframe(
    tl: list,
    ...
    edges_df["consumer_grouping_time"] = edges_df["consumer_date"].apply(
        lambda x: extract_grouping_date_as_string(temporal_grouping, x)
    )
    ...
    grouped_edges = (
        edges_df.groupby(
            ["producer_grouping_time", "consumer_grouping_time", "producer", "consumer"]
        )
        .agg({"amount": "sum"})
        .reset_index()
    )
    ...
    grouped_edges["date_producer"] = grouped_edges["producer_grouping_time"].apply(
        lambda x: convert_grouping_date_string_to_datetime(temporal_grouping, x)
    )  # date is date producer, but in long format
    grouped_edges["hash_producer"] = grouped_edges["date_producer"].apply(
        lambda x: extract_date_as_integer(x, time_res=temporal_grouping)
    ...
    grouped_edges = add_column_interpolation_weights_to_timeline(
        grouped_edges, database_date_dict, interpolation_type=interpolation_type
    )
...
def add_column_interpolation_weights_to_timeline(
    tl_df: pd.DataFrame, database_date_dict: dict, interpolation_type: str = "linear"
) -> pd.DataFrame:
    """
    ...
    def get_weights_for_interpolation_between_nearest_years(
        ...
        if closest_lower is None:
            print(
                f"Warning: Reference date {reference_date} is lower than all provided dates. Data will be taken from closest higher year."
            )
            return {closest_higher: 1}

        if closest_higher is None:
            print(
                f"Warning: Reference date {reference_date} is higher than all provided dates. Data will be taken from the closest lower year."
            )
    ...
    if interpolation_type == "nearest":
        ...
        tl_df["interpolation_weights"] = tl_df["interpolation_weights"].apply(
            lambda d: {database_date_dict[x]: v for x, v in d.items()}
        )
        return tl_df
